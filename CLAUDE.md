...
- `src/components/CalibrationScreen.jsx` — a fixation-based calibration screen (4 corners + center; standard for commercial eye trackers) shown after the camera starts and before painting is enabled. Each point is shown for `DWELL_MS` (2000ms) with a filling progress ring for feedback; every `SAMPLE_INTERVAL_MS` during the dwell, the point's known position is fed to `GazeTracker#calibrate` → `webgazer.recordScreenPosition(x, y, "click")` — no click needed, and continuous sampling during dwell yields more data per point than a single click. This is the *only* calibration data WebGazer gets now that its automatic mouse-based self-calibration is disabled — see the gotcha above. `src/App.jsx` renders this whenever `trackingMode === "webgazer" && !calibrated`; it auto-advances through points and calls `onComplete` itself after the last one (no button). Two prior calibration UIs were tried and reverted: click-to-calibrate (tedious) and a moving-dot smooth-pursuit sweep (also had a `requestAnimationFrame` timestamp/`performance.now()` ordering bug — see git history on `CalibrationScreen.jsx` if reviving that approach). This fixation version reuses the "derive `start` from the first rAF callback" fix either way since it has the same theoretical timing hazard.

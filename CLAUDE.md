...
- `src/lib/gazeTracker.js` — `GazeTracker` (extends `EventTarget`), emits `"gaze"` events with `{x, y, t}`. `start()` is async: it lazy-loads WebGazer.js from `https://webgazer.cs.brown.edu/webgazer.js` on first call (not in `index.html`, so the camera is never touched until a user explicitly triggers tracking — see `useColorReveal.js`'s `startTracking`), then calls `webgazer.begin()`; if that fails (no camera, permission denied, script blocked) it falls back to mouse position. `start()` resolves to `"webgazer"` or `"mouse"` so the UI can tell the viewer which mode is active. Smooths raw coordinates with a moving average (`config.gaze.smoothingWindow`).
...
- `src/hooks/useColorReveal.js` — the only place the three classes above are wired into React. It owns their lifecycle (construct on mount, resize handling, `gaze.stop()` on unmount) and exposes `{ loggerRef, startTracking }`. `startTracking()` is the one thing it does *not* do automatically — it must be called from a user gesture (a button click in `src/App.jsx`) since it's what triggers the camera permission prompt. `src/App.jsx` supplies refs to the line-art `<img>`, a hidden color-source `<img>` (used only as a `drawImage` source, never shown directly — see `.viewer__colorSource` in `src/index.css`), and the overlay `<canvas>`, then renders UI around what the hook reports: a start overlay before tracking begins, a "using mouse instead" banner if WebGazer failed, and a download button once `onCompleted` fires.

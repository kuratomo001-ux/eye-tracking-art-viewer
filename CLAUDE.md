...
- `src/lib/gazeTracker.js` — `GazeTracker` (extends `EventTarget`), emits `"gaze"` events with `{x, y, t}`. `start()` is async: it lazy-loads WebGazer.js from `https://webgazer.cs.brown.edu/webgazer.js` on first call (not in `index.html`, so the camera is never touched until a user explicitly triggers tracking — see `useColorReveal.js`'s `startTracking`), then calls `webgazer.begin()`. There is deliberately no mouse-position fallback — this app is gaze-only; if the camera/library isn't available, `start()` resolves to `"unavailable"` instead of silently switching input modes, and the UI shows a retry screen.
  ...
  Two more accuracy levers set before `.begin()`: `webgazer.params.camConstraints` requests `ideal: 1920x1080` webcam capture (WebGazer's own default is only `ideal: 640x480`) for a sharper eye/face image; and `.setRegression("weightedRidge")` instead of the default `"ridge"`, so recent calibration samples are weighted more heavily than old ones. (`webgazer.setTracker(...)`'s only valid value in this build is `"TFFacemesh"` and `setRegression`'s valid values are `"ridge"` / `"weightedRidge"` / `"threadedRidge"` — confirmed by reading the minified webgazer.js source, since none of this is in the sparse public docs.)
...
- `src/components/CalibrationScreen.jsx` — a smooth-pursuit calibration screen shown after the camera starts and before painting is enabled: a dot animates through a 9-point zigzag path (`WAYPOINTS`) and, every `SAMPLE_INTERVAL_MS`, feeds the dot's current (known) position to `GazeTracker#calibrate` → `webgazer.recordScreenPosition(x, y, "click")`. No clicking required, and since every sampled frame along the path is itself a labeled data point, this produces far more training samples than the click-based version it replaced (~60+ vs. 15). This is the *only* calibration data WebGazer gets now that its automatic mouse-based self-calibration is disabled — see the gotcha above. `src/App.jsx` renders this whenever `trackingMode === "webgazer" && !calibrated`; it calls `onComplete` itself once the path finishes (no button).

  **Gotcha:** the animation loop must derive its start time from the *first* `requestAnimationFrame` callback's own timestamp, not from a `performance.now()` called earlier in the same effect. rAF's timestamp is the frame's start time, which can be *before* a `performance.now()` call made synchronously afterward in the same tick — so `elapsed = firstCallbackTime - earlierPerformanceNow` can come out negative, sending an array index negative and crashing on destructuring `undefined`. Initialize `start` lazily inside the callback (`if (start === null) start = now`) instead.

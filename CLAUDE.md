...
- `src/lib/gazeTracker.js` — `GazeTracker` (extends `EventTarget`), emits `"gaze"` events with `{x, y, t}`. `start()` is async: it lazy-loads WebGazer.js from `https://webgazer.cs.brown.edu/webgazer.js` on first call (not in `index.html`, so the camera is never touched until a user explicitly triggers tracking — see `useColorReveal.js`'s `startTracking`), then calls `webgazer.begin()`. There is deliberately no mouse-position fallback — this app is gaze-only; if the camera/library isn't available, `start()` resolves to `"unavailable"` instead of silently switching input modes, and the UI shows a retry screen.
  ...
  Regression is `"ridge"` (all samples weighted equally), not `"weightedRidge"`. `weightedRidge` was tried briefly alongside an also-reverted fixation-based (gaze-and-dwell, no click) calibration screen that produced ~33 near-duplicate samples per point over a continuous 5s dwell; combined with `weightedRidge`'s recency weighting, "recent" effectively meant "the last calibration point looked at," skewing the fit toward wherever calibration happened to end and making tracking noticeably worse. Both changes were reverted back to `ridge` + the click-based `CalibrationScreen.jsx` described below — if a hands-free calibration UI is revisited, keep `ridge` (or verify a weighted variant doesn't reintroduce this bias) rather than assuming `weightedRidge` is a safe default.

...
- 「精度があまい」「クリックが面倒」との要望を受け、クリック方式のキャリブレーションを、点が自動で動きそれを目で追いかけるだけの**スムーズパシュート方式**に変更した(`src/components/CalibrationScreen.jsx`)。9点のジグザグ経路を9.6秒かけて移動する間、150ms間隔で「今この座標を見ているはず」というデータを自動的に`webgazer.recordScreenPosition`へ渡す。クリック不要な上、経路全体が連続的にラベル付きデータになるため、従来のクリック方式(15クリック)よりずっと多いサンプル数(60件以上)を確保できる。
  - 実装中、`requestAnimationFrame`のタイムスタンプが「フレーム開始時刻」であり、同じ処理内で後から呼んだ`performance.now()`より前の時刻になることがあるという落とし穴に当たった。これにより`elapsed`が負になり配列に負インデックスでアクセスして`undefined`を分割代入しクラッシュしていた。原因調査の過程で一時的に`weightedRidge`を`ridge`に戻すなど回帰アルゴリズム側を疑ったが無関係で、`start`(基準時刻)を最初の`tick`コールバック自身のタイムスタンプから取るように直して解決した。Playwrightで9.6秒のアニメーション完走・クラッシュ無しを確認済み。

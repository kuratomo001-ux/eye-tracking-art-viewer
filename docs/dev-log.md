...
- 仮のSVG画像を、著作権フリーの実在の絵画に差し替えた。Van Gogh「星月夜」(1889年、パブリックドメイン)をWikimedia Commonsから取得(900x712にリサイズ、`artwork-color.jpg`)。線画版(`artwork-lineart.png`)は、Node.js + `sharp`で書いた使い捨てスクリプトでSobelフィルタによるエッジ検出(グレースケール化→ぼかし→勾配の大きさをsqrt(gx²+gy²)で手動計算→相対しきい値で二値化→反転)を行って生成した。しきい値は複数試し(0.12/0.2/0.3/0.4)、0.28に決定: 低いと全ての筆致まで線になり線画だけで絵が完成して見えてしまい(色彩の「発見」効果が薄れる)、高すぎると渦や糸杉の輪郭が消えるため、両方の中間で構図が伝わる密度を選んだ。Playwrightで線画表示・カラーレイヤーの読み込みを確認済み(視線推定自体はフェイクカメラのため未検証)。生成スクリプトはリポジトリには含めていない(一回限りの前処理のため)。
